...
def test_booking_rejects_past_dates(user, property_obj):
    ...
    booking = Booking(user=user, property=property_obj, check_in=yesterday, check_out=tomorrow, guests=1)
...
def test_booking_rejects_checkout_before_checkin(user, property_obj):
    tomorrow = timezone.localdate() + timedelta(days=1)
    booking = Booking(user=user, property=property_obj, check_in=tomorrow, check_out=tomorrow, guests=1)
...
def test_booking_rejects_overlapping_dates(user, property_obj):
    ...
    Booking.objects.create(user=user, property=property_obj, check_in=start, check_out=end, guests=1)
    overlap = Booking(user=user, property=property_obj, check_in=start + timedelta(days=1), check_out=end, guests=1)
...
def test_booking_calculates_totals(user, property_obj):
    ...
    booking = Booking(user=user, property=property_obj, check_in=start, check_out=start + timedelta(days=2), guests=1)
